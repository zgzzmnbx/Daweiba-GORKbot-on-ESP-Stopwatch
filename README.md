...
> 当前工程版本：v0.3.0（应用区已烧录并读回校验；BLE 已连接并完成表情回执验证）  
> 初始化日期：2026-09-16
...
- 当前使用对象：石萌（大尾巴）
- 当前目标：保留 StopWatch 已验证的硬件控制底座，以大尾巴导出的 Bible Strong `Grok bot` 为视觉和动画数据源，并在 v0.3.0 增加受绑定保护的 BLE 表情控制；v0.3.0 应用区已写入并读回校验，BLE 已完成配对、连接和表情回执验证。
- 最终交付物：可维护源码、可复现构建环境、Grok 风格几何资产、固件构建产物、测试记录、恢复说明和使用文档。

## 2. 当前状态

- 当前工程版本：v0.3.0；BLE 固件、Bluetooth 设置页、Windows Bleak 控制台、应用区写入和 G4 核心无线验收已完成。已验证加密连接、服务发现、状态通知、菜单忙保护、23/23 表情命令和一次重连；陌生端拒绝、10 次重连、P95/FPS/30 分钟稳定性仍待补测。
- 当前阶段：G4 收敛——v0.3.0 已完成核心 BLE 连接和表情控制；v0.2.5 真机显示、外设和长期稳定性验收仍可并行进行。
...
- 当前主线任务：按 `00-docs/00-PRD/07-v0.3.0-BLE表情操控开发PRD-【codex】.md` 完成 BLE 表情控制的 G4 真机门禁；同时保留 v0.2.5 真机功能验收。
...
- 下一步优先事项：补做未绑定控制端拒绝、10 次重连、命令延迟、BLE 连接下 FPS、30 分钟稳定性和 USB 回退验收。
