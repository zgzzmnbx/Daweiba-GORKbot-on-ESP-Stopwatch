...
> 当前开发版本：v0.7.0-P0（纯 BLE 设备音频测试版）；电脑语音工作台仍 v0.6.0，设备应用区仍 v0.5.0，本轮未烧录。
> 初始化日期：2026-09-16
...
入口：[start-ble-audio-test.cmd](start-ble-audio-test.cmd)，默认 echo 测试。需先获授权烧录新镜像；旧 v0.5.0 设备还不能使用。已通过模拟测试与固件编译，停在 G1 烧录门禁，尚未证明真机有声或 BLE 吞吐。现有 8766 页面仍是电脑录音/回读，尚未接设备音频。详见 [操作与协议](00-docs/00-PRD/12-v0.7.0-P0音频探针协议与操作-【codex】.md)、[实施记录](04-output/v0.7.0/实施与验收记录-【codex】.md) 和 [完整 PRD](00-docs/00-PRD/11-v0.7.0-纯BLE设备录放音方案-【codex】.md)。
...
先启动独立语音服务，再双击项目根目录 [start-voice-companion.cmd](start-voice-companion.cmd)，打开 `http://127.0.0.1:8766`。默认本地识别/回读，两个上传许可关闭；StopWatch 离线时电脑端仍可使用。设备正常开机并开启 BLE，首次点 Pair，退出设置页后在网页扫描连接；不要同时开旧蓝牙控制台。详见 [工作台使用说明](03-Src/stopwatch-voice-companion/README.md) 和 [本轮验收记录](04-output/v0.6.0/实施与验收记录-【codex】.md)。
...
- 当前电脑端版本：v0.6.0；源码在 `03-Src/stopwatch-voice-companion/`。保留原命令行入口，将 BLE 协议抽取到 `tools/stopwatch_ble.py`；补齐严格回执、超时隔离与工作台有限重连。
- 当前固件源码：v0.7.0-P0；设备应用区仍为 v0.5.0。本轮新增独立音频测试页和 GATT 服务，保留 `:say` / `:clear`、`happy-work` 及既有 UUID；编译通过但未烧录。原设备实屏、帧率及 BLE 安全/长期稳定性门禁继续保留。
...
- 当前主线任务：按 `00-docs/00-PRD/10-v0.6.0-电脑语音工作台-【codex】.md` 完成真人麦克风、听感与 BLE 联动验收；不把自动测试、HTTP 回执计作真实屏幕效果。
