...
class BleExpressionClient:
    ...
    async def send_command(self, text: str) -> str:
        ...
        name = {"sleepy": "drowsy", "dizzy": "playful"}.get(name, name)
        async with self._write_lock:
            return await self._send_packet(command.encode("ascii"), "OK:" + name.upper())

    async def _send_packet(self, payload: bytes, expected_status: str) -> str:
        ...
        try:
            ...
            await self.disconnect()
            raise BleConsoleError("BLE 写入失败；显示结果未确认") from exc
        deadline = asyncio.get_running_loop().time() + self._status_timeout
        while True:
            remaining = deadline - asyncio.get_running_loop().time()
            if remaining <= 0:
                await self.disconnect()
                raise BleConsoleError("等待设备回执超时；结果未确认，已断开以隔离迟到回执")
            try:
                status = await asyncio.wait_for(self._status_queue.get(), remaining)
            except asyncio.TimeoutError as exc:
                await self.disconnect()
                raise BleConsoleError("等待设备回执超时；结果未确认，已断开以隔离迟到回执") from exc
            if status == expected_status:
                return status
            if status.startswith("ERR:"):
                if status == "ERR:BAD_CMD" and payload[0] >= TEXT_BEGIN:
                    raise BleConsoleError("设备不支持文字气泡；请先写入 v0.5.0 固件")
                raise BleConsoleError(f"设备拒绝：{status}")
    ...
    async def send_text(self, text: str) -> str:
        async with self._write_lock:
            ...
            for payload, expected in packets:
                receipt = await self._send_packet(payload, expected)
            return receipt

    async def clear_text(self) -> str:
        async with self._write_lock:
            return await self._send_packet(bytes((TEXT_CLEAR,)), "OK:CLEAR")
